...
RUN python -m venv /opt/world-parser-env
ENV PATH /opt/world-parser-env/bin:$PATH
ENV VIRTUAL_ENV /opt/world-parser-env

# Set the working directory.
WORKDIR /workspaces/world-parser/

# Install the run time Python dependencies in the virtual environment.
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/world-parser/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/world_parser/ && touch src/world_parser/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/world-parser/
